...
**現在の対応言語：** 英語（`en`、既定）、日本語（`ja`）、韓国語（`ko`）、簡体字中国語（`zh`）、繁体字中国語（`zh-hant`）。
日本語・韓国語・中国語（簡体字/繁体字）はいずれも `bge-m3` の多言語埋め込みモデルを使用します（英語は `nomic-embed-text`）。
韓国語の要約モデルは `exaone3.5`、中国語（簡体字・繁体字）の要約モデルは `ornith-1.5:9b` です（繁体字は 繁體/正體字 での出力を強制するプロンプトで駆動します）。キーワード（FTS5）検索では、CJK テキスト（日本語・韓国語・中国語）を
標準の `unicode61` トークナイザに加え、アプリ側の文字バイグラム分割（`cjk.py` を参照）で
インデックス化します。FTS5 組み込みの `trigram` トークナイザではありません。日本語は語間にスペースが無く、
韓国語は膠着語、中国語はそもそも語境界が無いため、インデックス時とクエリ時の両方のテキストを、FTS5 に渡す前に重なり合う
2 文字バイグラムへ事前分割します。同じ分割器が簡体字・繁体字の両方をカバーします（どちらも CJK 統合漢字の範囲にあるため、個別の処理は不要です）。これは形態素解析器ではなく（MeCab/jieba/konlpy への依存は
ありません）、言語的な正確さを引き換えに、2 文字以上のあらゆる CJK 部分文字列を確実にインデックス化する、
依存ゼロの代替手段です。1 文字の CJK クエリは Keyword モードでは完全一致しません（設計上 — 1 文字一致は
ノイズが多すぎるため）が、Both/意味検索では引き続き提示されます。

**韓国語のアルファベット索引バー：** 韓国語は真のアルファベット（かな・漢字には頭文字順が無い日本語とは
異なります）であるため、文書一覧の索引バーが韓国語で有効になり、14 個の基本初声（チョソン）
ㄱ ㄴ ㄷ ㄹ ㅁ ㅂ ㅅ ㅇ ㅈ ㅊ ㅋ ㅌ ㅍ ㅎ を表示します。各ボタンは、その子音で始まる音節で始まる
タイトルの文書を絞り込みます（濃音は基本子音に統合されます。例：ㄲ→ㄱ）。英語は A–Z/0–9 バーを使用し、
日本語と中国語（簡体字・繁体字とも）ではバーは非表示のままです。中国語はアルファベットではなく頭文字順が無いため、日本語と同じ理由で非表示になります。

**言語の選択：**

- **インストール時** — 新規インストール時に `install.sh` が言語を尋ねます。非対話的に答えるには
  `DOCUBROWSE_LANG=en`、`=ja`、`=ko`、`=zh`、`=zh-hant` を使います。**アップグレードでは再質問されません** —
  `docubrowse.config` の既存の `lang` 値は常に保持されます。（プラットフォームインストーラ —
  Windows/macOS/RPM/DEB — は新規インストールを既定で `lang = en` にします。あとから設定で
  変更してください。）
- **起動時** — `docubrowser ko start`（または `docubrowser start ja`）でその言語でサーバーを
  起動します。コードはどちらの位置に置いても構いません。選択は `docubrowse.config` に書き込まれ、
  以降のプレーンな `docubrowser start` でも別の言語を選ぶまで維持されます。
- **インストール後** — Web UI を開き、設定（歯車）アイコンをクリックし、全般パネルの言語ドロップダウンを
  使います（`POST /api/language` を呼び出します）。異なる埋め込みや FTS トークナイザを使う言語へ切り替える
  場合（例：英語 ↔ 日本語/韓国語）は警告が表示されます。既存の文書は、新しい言語向けに再構築するため
  `docubrowser rescan`（または `embed_docs.py`）を再実行するまで、古い埋め込みと FTS インデックスを
  保持します。それまではインデックス済み文書の検索品質が低下します。（日本語 ↔ 韓国語は `bge-m3`
  埋め込みと同じトークナイザを共有するため、この 2 言語間では再構築は不要です。）

**まだ未対応：** 混在言語または文書単位の言語コーパス、英語/日本語/韓国語/中国語以外の言語（`lang_models.py` の
`LANG_MODELS` テーブルの 1 行と `locales/<code>.json` ファイルが仕組みのすべてであり、追加はコード変更
ではなくデータ変更です）、日本語/中国語の文書一覧のためのかな/読みベース（またはピンイン）索引バー
（どちらも頭文字順が無く、韓国語は上記の初声バーを使用します）、日本語の「マイナンバー」PII 検出
（現状は米国 PII パターンのみ実装）。
...
| 混在言語コーパス非対応 | 1 インストールは 1 言語を提供します（英語・日本語・韓国語・簡体字中国語・繁体字中国語、インストール時または設定で選択）。文書単位の言語検出や混在言語コーパスには対応しません。[言語](#languages)を参照 |
...
- **言語切り替え後のセマンティック検索の HTTP 500 を修正。** セマンティック
  スコアリングは現在アクティブな埋め込みモデルで生成されたベクトルのみを比較する
  ため、別の言語で構築されたインデックスはクラッシュせずキーワード検索に
  フォールバックします。
- **ディープリンクが CJK の該当箇所を見つけられるように。** キーワードの
  ディープリンクは検索インデックスと同じ文字バイグラムにクエリを分割するため、
  検索で見つかった文書は必ず該当箇所を返します（以前は 100% 一致でも「該当する
  箇所が見つかりません」と表示されていました）。
- **設定した言語がすべての段階で反映されるように。** `docubrowser scan`、
  埋め込み生成、データベース初期化がサーバーと同じ言語を解決するようになり、
  設定した `ko`/`ja` が英語で黙ってインデックスされることがなくなりました。言語を
  設定すれば `docubrowser ko scan` は不要です。

## v1.5.0 (2026-09-11) — 韓国語対応 + UI の完全ローカライズ

DocuBrowse が完全に**韓国語**で動作するようになり、インターフェースのローカライズが
完成しました（検索ページと設定ページ）。[言語](#languages)を参照。

- **韓国語（`ko`）。** 完全な言語スタック：`bge-m3` 埋め込み、`exaone3.5` 要約、
  ハングルの文字バイグラムによるキーワード検索、完全な韓国語 UI 翻訳（`locales/ko.json`）。
- **韓国語のアルファベット索引バー。** ハングルは真のアルファベットなので、文書一覧の
  索引バーが韓国語で有効になり、14 個の基本初声（チョソン）ㄱ–ㅎ で閲覧できます。濃音は
  基本子音に統合されます（ㄲ→ㄱ）。英語は A–Z/0–9 バーを維持し、日本語ではバーは
  非表示のままです。
- **設定ページを完全にローカライズ。** 設定ページのすべてのラベル、説明、ボタン、
  プレースホルダ、ステータス行、確認ダイアログ、トースト、アラートがローカライズ
  されました（英語/日本語/韓国語）。
- **CLI から言語を指定して起動。** `docubrowser ko start`（または `docubrowser start ja`）で
  その言語でサーバーを起動し、選択を `docubrowse.config` に保存します。コードはどちらの
  位置に置いても構いません。
- **ローカライズされたメッセージ。** 要約モーダルのエラー、文書/タグのカウンター、
  タグ分類（code/diagram/markup）、トップへ戻るボタン、ネットワーク/エラーメッセージが
  翻訳されました。
- **修正。** 新規インスタンスの「ディレクトリ追加エラー」（空の `work_dir` 行が既定値を
  上書きしなくなりました）、CLI が設定 UI の書き込む構成と同じものを読むようになり
  `scan` が構成済みディレクトリを対象にします、HTML を `no-cache` で配信し UI/ローカライズの
  変更が常に新しく読み込まれます。
