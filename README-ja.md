...
**現在の対応言語：** 英語（`en`、既定）、日本語（`ja`）、韓国語（`ko`）。
日本語と韓国語はどちらも `bge-m3` の多言語埋め込みモデルを使用します（英語は `nomic-embed-text`）。
韓国語の要約モデルは `exaone3.5` です。キーワード（FTS5）検索では、CJK テキスト（日本語・韓国語）を
標準の `unicode61` トークナイザに加え、アプリ側の文字バイグラム分割（`cjk.py` を参照）で
インデックス化します。FTS5 組み込みの `trigram` トークナイザではありません。日本語は語間にスペースが無く、
韓国語は膠着語であるため、インデックス時とクエリ時の両方のテキストを、FTS5 に渡す前に重なり合う
2 文字バイグラムへ事前分割します。これは形態素解析器ではなく（MeCab/jieba/konlpy への依存は
...
- **インストール時** — 新規インストール時に `install.sh` が言語を尋ねます。非対話的に答えるには
  `DOCUBROWSE_LANG=en`、`=ja`、`=ko` を使います。**アップグレードでは再質問されません** —
...
| 混在言語コーパス非対応 | 1 インストールは 1 言語を提供します（英語・日本語・韓国語、インストール時または設定で選択）。文書単位の言語検出や混在言語コーパスには対応しません。[言語](#languages)を参照 |
