...
def test_two_char_chinese_query_matches(monkeypatch):
    """Chinese has no word boundaries: 深度链接 (Deep Links) is one run.
    Bigram segmentation lets the 2-char term 链接 match inside it."""
    monkeypatch.setenv("DOCUBROWSE_LANG", "zh")
    docubrowse_db._LANG_CACHE.clear()
    try:
        with tempfile.TemporaryDirectory() as tmp:
            db = str(Path(tmp) / "cjk_zh.db")
            conn = docubrowse_db.get_db(db)          # unicode61 for zh
            snippet = cjk_segment("这份文档介绍深度链接功能与语义搜索")
            conn.execute(
                "INSERT INTO doc_fts(rowid, name, title, author, subject, description, content_snippet, tags) "
                "VALUES (1,'','', '', '', '', ?, '')", (snippet,))
            q = cjk_segment("链接")
