...
- Prose formats: **PDF, TXT, HTML, Markdown, DOCX, RTF, ODT**, e-books (EPUB, MOBI, AZW3), DjVu, plus the SGML/XML family (XHTML, XML, DocBook, RSS/Atom, OPML) and other text/markup formats (reST, AsciiDoc, LaTeX, config files, JSON/YAML, email, source code). Non-prose (spreadsheets, presentations, diagrams) fall back to opening in their reader.
...
- **Deep Links — in-document passage search.** From any keyword or semantic
  ...
  (EPUB/MOBI/AZW3), DjVu, plus the SGML/XML family, email, JSON/YAML, source
  code, and other text/markup formats; non-prose falls back to opening in the
  reader. New endpoint `GET /api/deep-links`.
