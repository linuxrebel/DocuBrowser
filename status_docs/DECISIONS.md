...
| Remove "embedded" count from header (deferred) | Header stats currently show "N docs · N embedded · N tags". Remove the "X embedded" portion — future cosmetic tweak. | 2026-06-11 |
...
| Additional Scan Directories panel | New settings panel (simplified clone of Ignored Directories UX) lets users maintain a list of extra top-level directories to scan, stored in `scan_dirs.txt` (next to the DB, one absolute path per line, '#' comments) via `_load_scan_dirs()`/`SCAN_DIRS_FILENAME` in scan_docs.py and new `/api/scan-dirs` GET/POST endpoints in doc_search.py. Adding a directory via Browse… (live-synced path, no "select this") shows an alert reminding the user it is NOT scanned automatically, with the exact CLI command to run: `cd "<workDir>" && python3 docubrowser.py scan --doc-dir "<path>" --limit 100` — re-running the same command resumes with the next 100 unindexed files. Removing an entry requires confirm. Verified via Playwright: panel renders, browse live-sync works, add shows correct alert text and updates the list, remove confirms and clears back to "No additional scan directories configured", 0 console errors. | 2026-06-11 |
