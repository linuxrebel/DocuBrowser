...
**현재 지원:** 영어(`en`, 기본), 일본어(`ja`), 한국어(`ko`), 중국어 간체(`zh`), 중국어 번체(`zh-hant`).
일본어·한국어·중국어(간체/번체)는 모두 `bge-m3` 다국어 임베딩 모델을 사용합니다(영어는 `nomic-embed-text`).
한국어 요약 모델은 `exaone3.5`, 중국어(간체·번체) 요약 모델은 `ornith-1.5:9b` 입니다(번체는 繁體/正體字 출력을 강제하는 프롬프트로 구동됩니다). 키워드(FTS5) 검색에서 CJK 텍스트(일본어·한국어·중국어)는
FTS5 내장 `trigram` 토크나이저 대신 표준 `unicode61` 토크나이저와 앱 측 문자 바이그램
분절(`cjk.py` 참조)로 인덱싱됩니다 — 일본어 텍스트는 단어 사이에 공백이 없고, 한국어는
어절 사이에 공백이 있지만 교착어라 명사에 조사가 붙으며(예: `학교에서` = `학교` + `에서`), 중국어는 단어 경계가 아예 없으므로,
색인 시점과 질의 시점 텍스트 모두 FTS5 가 보기 전에 겹치는 2 문자 바이그램으로 미리
분할됩니다. 같은 분절기가 간체·번체를 모두 처리합니다(둘 다 CJK 통합 한자 범위에 있어 별도 처리가 필요 없습니다). 이는 형태소 분석기가 아니며(MeCab/jieba/konlpy 의존성 없음), 언어적 정확성을
...
영어는 A–Z/0–9 바를 사용하고, 일본어와 중국어(간체·번체 모두)에서는 바가 숨겨진 채로 유지됩니다 — 중국어는 알파벳이 아니고 첫 글자 순서가 없어, 일본어와 같은 이유로 숨겨집니다.

**언어 선택:**

- **설치 시점** — `install.sh` 가 새 설치에서 언어를 묻습니다. 비대화식으로는
  `DOCUBROWSE_LANG=en`, `=ja`, `=ko`, `=zh`, `=zh-hant` 로 답합니다. **업그레이드는 절대 다시 묻지
...
| 혼합 언어 코퍼스 없음 | 한 설치는 하나의 언어를 제공(영어·일본어·한국어·중국어 간체·중국어 번체, 설치 시점 또는 설정에서 선택); 문서 단위 언어 감지나 혼합 언어 코퍼스는 미지원. [언어](#languages) 참조 |
...
- **언어 전환 후 의미 검색의 HTTP 500 오류 수정.** 의미 점수 계산이 현재 활성
  임베더로 생성된 벡터만 비교하므로, 다른 언어로 구축된 인덱스는 충돌하지 않고
  키워드 검색으로 대체됩니다.
- **딥 링크가 CJK 구절을 찾도록 수정.** 키워드 딥 링크가 검색 인덱스와 동일한
  문자 바이그램으로 질의를 분할하므로, 검색으로 노출된 문서는 항상 일치하는
  구절을 반환합니다(이전에는 100% 일치에서도 "일치하는 구절 없음"이
  표시되었습니다).
- **설정한 언어가 모든 단계에서 적용.** `docubrowser scan`, 임베딩, 데이터베이스
  초기화가 서버와 동일한 언어를 사용하도록 하여, 설정한 `ko`/`ja` 가 영어로
  조용히 인덱싱되지 않습니다. 언어를 설정하면 `docubrowser ko scan` 은 더 이상
  필요하지 않습니다.
