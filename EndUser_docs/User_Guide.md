...
It works on text documents — **PDF, TXT, HTML, Markdown, DOCX, RTF, ODT**, e-books (EPUB, MOBI, AZW3), DjVu, and most other text and markup formats (XML, RSS/Atom, DocBook, reStructuredText, AsciiDoc, LaTeX, and config files). (DRM-protected e-books can't be read, so those still open in their reader.) Spreadsheets, presentations, and diagrams don't display as flowing text, so for those Deep Links offers to open the file in its normal reader instead.
