...
It works on text documents — **PDF, TXT, HTML, Markdown, DOCX, RTF, ODT**, and most other text and markup formats (XML, RSS/Atom, DocBook, reStructuredText, AsciiDoc, LaTeX, and config files). Spreadsheets, presentations, and diagrams don't display as flowing text, so for those Deep Links offers to open the file in its normal reader instead.
