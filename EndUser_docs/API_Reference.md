...
The path must already be in the index (the endpoint validates it and checks the file exists on disk before reading it). Prose formats — **PDF, TXT, HTML, DOCX, RTF, ODT**, e-books (**EPUB, MOBI, AZW3, AZW** — DRM-encrypted files yield no passages), **DjVu**, the SGML/XML family (XHTML, XML, SGML, DocBook, RSS, Atom, OPML), and plain-text/markup (Markdown, reStructuredText, AsciiDoc, LaTeX, config-ish text `.ini`/`.conf`/`.cfg`/`.log`/`.lst`, JSON/YAML/TOML, email `.eml`, and source code) — return passages; non-prose formats (spreadsheets, presentations, diagrams) return `unsupported`.
